...
- Path requests now carry an authenticated 10-second timestamp. Strictly sized
  requests with unknown CIDs are dropped before HMAC verification; requests for
  known CIDs allocate no pre-authentication state or quota. Invalid HMACs cost
  stateless CPU work only, while valid HMACs consume the strict per-CID response
  quota.
