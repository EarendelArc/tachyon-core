...
Relay 拒绝已过期或超过未来 30 秒的 Hello。客户端对同一个认证 Hello 最多发送 4 次：
首次发送加 3 次丢包恢复，接收窗口依次为 100 ms、200 ms、400 ms 和 800 ms。
每次发送及退避都受调用方 deadline 与配置 deadline 中较早者约束；一旦到达这个
绝对截止时间就立即结束，不等待 context 定时器异步更新。相同认证 Hello 最多重放
3 次缓存 ACK，与客户端剩余尝试次数一一对应，且缓存严格在 Hello 签名的过期时间
失效。结束会话的 SessionID 进入最多 4096 条、
...
处理顺序固定为严格报文长度/类型、廉价 session map 查询、时间窗、HMAC，然后重新获取 session 锁并复核 session 身份、迁移策略、active endpoint 和来源 owner。未知 `SessionID` 在 HMAC 前直接丢弃；畸形、过期或 HMAC 无效的请求不分配状态、不消耗迁移 token，也不发送响应。只有 HMAC 有效且锁内复核通过的请求才会扣除该 CID 的 session bucket（burst 8、每秒恢复 2）并生成 challenge。因此无效请求保持常数内存，也不能剥夺合法迁移配额。明确的 DoS 权衡是：知道活跃 `SessionID` 的攻击者仍可制造无状态 HMAC CPU 工作；协议刻意不设置可被未认证流量耗尽的共享 limiter。时间窗内捕获的有效请求能够消耗认证后的每会话配额并产生有界 challenge 响应。重复响应会被已消费 cookie 检查拒绝。未知非控制数据仍然 fail-closed，不会广播给所有活跃 session 尝试解密。
